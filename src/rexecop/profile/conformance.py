...
def validate_profile_conformance(
    ...
    *,
    ...
    if checked_track == "mutation" and not checked_intents:
        errors.append("mutation_track:no_mutation_candidates")
...
def _validate_mutation_candidate(
    *,
    ...
    if not modes:
        errors.append(f"{operation_id}:mutation_candidate:modes_missing")
    if modes and modes <= READ_ONLY_MODES:
        errors.append(f"{operation_id}:mutation_candidate:not_mutating_modes:{sorted(modes)}")
    if side_effect_class == "none":
        errors.append(f"{operation_id}:mutation_candidate:side_effect_class:none")
    facts_contract = str(metadata.get("facts_contract") or "").strip()
    if not facts_contract:
        errors.append(f"{operation_id}:mutation_candidate:facts_contract_missing")
    ...
    if not runbook_ref:
        errors.append(f"{operation_id}:mutation_candidate:runbook_ref_missing")
    if not validation_ref:
        errors.append(f"{operation_id}:mutation_candidate:validation_ref_missing")
