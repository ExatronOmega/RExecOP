...
RExecOp `0.2.9a0` is a published alpha line for operator evaluation. This document states
...
| Source alpha line | `0.2.9a0` is current on `main`; see [CHANGELOG.md](../CHANGELOG.md) for history |
| Coordinated dependencies | Requires published `govengine>=0.16.2,<0.17`; the `tecrax` extra requires `tecrax>=0.3.7a0,<0.4` |
