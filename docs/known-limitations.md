...
RExecOp `0.2.21a0` is the current source alpha line for operator evaluation.
...
| Limitation | Detail |
| --- | --- |
| Public PyPI | `rexecop==0.2.21a0` published for alpha evaluation — not a production-ready claim |
| Source alpha line | `0.2.21a0` is current on `main`; see [CHANGELOG.md](../CHANGELOG.md) for history |
| Coordinated dependencies | Source line requires `govengine==0.16.9` and `sclite-core==1.0.8`; the `tecrax` extra requires `tecrax==0.3.15a0` |
