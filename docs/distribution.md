...
RExecOp `0.2.23a0` is the current published alpha line on
...
| Path | When to use |
| --- | --- |
| **PyPI** (`pip install rexecop==0.2.23a0`) | Evaluation of the single supported alpha line |
...
```bash
python -m pip install "rexecop[tecrax]==0.2.23a0"
```
...
The current RExecOp source line and published `0.2.23a0` wheel require GovEngine `0.16.9` and SCLite `1.0.8`.
